...
| AI 批改与辅助 | ✅ 完成 | 自有 Key + 免费通道、提交批改、**边写边辅助（可 SSE 流式）**、追问、用量统计 |
...
3. **AI 闭环** — 配置 Key 或免费通道 → 边写边辅助 / 提交批改 → Token 用量可见。
...
| AI | 自备 Key 或免费配额；实时辅助支持流式与跨批去重 | 增量段落请求、更细的失败重试与批改体验 |
...
- **写作辅助面板**（桌面右侧 / 手机浮动可拖动）
  - 写作计时：预设 / 自定义 / 倒计时 / 正计时
  - **AI 辅助开关**（辅助面板内保存）：提交后批改项 + **实时辅助写作**
  - **实时辅助**：停笔约 2–3 秒后请求建议，按时间分批追加；可与历史批次去重
  - **流式输出**（可选）：边生成边展示；展示效果支持逐条淡入 / 打字机 / 整体淡入
  - 建议为会话级展示，不落库；点「重写」或清空可清除当前建议
- 专注写作时可锁定页面切换
...
- 或走后端免费通道（需管理员开启）
- 用途包括：
  - **提交后**：语法检查、结构评分、词汇 / 提升建议、建议追问
  - **写作中**：`realtime_assist` 实时短建议（HTTP 或 SSE 流式）
  - 能力测评评估等
- Token 用量可在用户中心查看
- 冷启动「加载中」文案随本地界面语言适配
